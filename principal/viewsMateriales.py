...
def modificarMateriales(request):
    m = Materiales(idMaterial=request.POST['idMaterial'])
...
def modificarM(request):
    registro = Materiales.objects.get(idMaterial=request.POST['idMaterial'])
    return render(request,'modificarMateriales.html',{'reg':registro})

def eliminarM(request):
    id = request.GET['idMaterial']
    return render(request,'eliminarMaterial.html',{'id':id})
